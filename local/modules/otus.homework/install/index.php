...
class otus_homework extends CModule // точку заменить на _
{
    ...
    function __construct() // PЗдесь указываются данные для установочника из админской панели - Marketplace - Установленные решения
    {
        $arModuleVersion = array();
        include(__DIR__ . '/version.php'); // Описывает переменную $arModuleVersion
        ...
        // Это всё обязательная информация для публикации на маркетплейсе
    }
    ...
    public function InstallDB()
    {
        ...
        $ib = new \CIBlock;
        $iblockId = $ib->Add($arFields); // старое ядро
    ...
    public function InstallEvents()
    {
        ...
        // нужен чтобы добавить новый обработчик

        // добавляем к обработчку событии присеодинять к существующим вкладкам свои вкладки
        // registerEventHandler - Добавляем новый обработчик
        $eventManager->registerEventHandler(
            'crm', // событие какого модуля мы обрабатываем
            'onEntityDetailsTabsInitialized', // событие какого типа мы обрабатываем
            $this->MODULE_ID, // какой модуль будет это всё обрабатывать
            '\\Otus\\Homework\\Crm\\Handlers', // какой неймспейс будет обрабатывать
            'updateTabs' // метод, который будет выполнять обработку
        );
